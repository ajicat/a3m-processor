...
inline void ConvPerChannel(
    ...
    for(int batch=0; batch<kernel_shape[0]; batch++){
        ...
        for(int real_tile=0; real_tile<tile_number; real_tile++){
            ...
            */
            ...
            //                    reinterpret_cast<uintptr_t>(o_data)));
            // RawNewlineD();

            //RawPutcD('['); RawPutcD('C'); RawPutcD('S'); RawPutcD('I'); RawPutcD(']');
            // RawTagHexD('0', static_cast<uint32_t>(i_shape.Dims(0)));
            // RawTagHexD('1', static_cast<uint32_t>(i_shape.Dims(1)));
            //RawTagHexD('2', static_cast<uint32_t>(input_width));
            // RawTagHexD('3', static_cast<uint32_t>(i_shape.Dims(3)));
            // RawNewlineD();

            //RawPutcD('['); RawPutcD('C'); RawPutcD('S'); RawPutcD('O'); RawPutcD(']');
            // RawTagHexD('0', static_cast<uint32_t>(o_shape.Dims(0)));
            // RawTagHexD('1', static_cast<uint32_t>(o_shape.Dims(1)));
            //RawTagHexD('2', static_cast<uint32_t>(output_width));
            // RawTagHexD('3', static_cast<uint32_t>(o_shape.Dims(3)));
            // RawNewlineD();

            // RawPutcD('['); RawPutcD('C'); RawPutcD('S'); RawPutcD('F'); RawPutcD(']');
            // RawTagHexD('0', static_cast<uint32_t>(w_shape.Dims(0)));
            // RawTagHexD('1', static_cast<uint32_t>(w_shape.Dims(1)));
            // RawTagHexD('2', static_cast<uint32_t>(w_shape.Dims(2)));
            // RawTagHexD('3', static_cast<uint32_t>(w_shape.Dims(3)));
            // RawNewlineD();
            ...
            //RawTagHexD('S', static_cast<uint32_t>(output_shift[0]));
                    ...
                    for(int width=0; width<padded_input_shape[2]; width++){
                        for(int channel=0; channel<input_shape[3]; channel++){
                            if((height-padding_up<0)||(width-padding_left<0)||(height+padding_down==padded_input_shape[1])||(width+padding_right==padded_input_shape[2])){
                                ...
                                =i_data[(((height-padding_up))*input_shape[2]+(width-padding_left))*input_shape[3]+channel];
                            }
                        }
                    }
                }
            ...
            for (int batch = 0; batch < batches; ++batch) {
                for (int out_y = 0; out_y < output_height; ++out_y) {
                const int in_y_origin = (out_y * stride_h)-pad_height;
                for (int out_x = 0; out_x < output_width; ++out_x) {
                    const int in_x_origin = (out_x * stride_w)-pad_width;
                    for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
                    auto group = out_channel / filters_per_group;
                    int32_t acc = 0;
                    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                        ...
                        for (int in_channel = 0; in_channel < filter_input_depth;
                            ++in_channel) {
                            int32_t input_val =
                                padded_image[((batch * input_height + in_y) * input_width + in_x) * input_depth + in_channel + group * filter_input_depth];
                            int32_t filter_val = w_data[((out_channel * filter_height + filter_y) * filter_width + filter_x) * filter_input_depth + in_channel];
                            acc += filter_val * (input_val + input_offset);
                        }
                        }
                    }
                    ...
                    if (b_data) {
                        acc += b_data[out_channel];
                    }
                    acc = MultiplyByQuantizedMultiplier(
                        acc, output_mult[out_channel], output_shift[out_channel]);
                    acc += output_off;
                    acc = std::max(acc, output_activation_min);
                    acc = std::min(acc, output_activation_max);
                    o_data[((batch * output_height + out_y) * output_width + out_x) * output_height + out_channel] =
                        static_cast<int8_t>(acc);
                    }
                }
                }
    ...
    */
    RawPutcD('['); RawPutcD('P'); RawPutcD('W'); RawPutcD('O'); RawPutcD(']');
