                ...
                for (int out_x = 0; out_x < output_width; ++out_x) {
                    ...
                    for (int m = 0; m < depth_multiplier; ++m) {
                        ...
                        for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                            ...
                            int32_t input_val = padded_image[((batch * input_height + in_y) * input_width + in_x) * input_depth + in_channel];
                            int32_t filter_val = w_data[((0 * filter_height + filter_y) * filter_width + filter_x) * w_tile_shape[3] + output_channel];
                            acc += filter_val * (input_val + input_offset);
                            }
                        }
                        }
                        if (b_data) {
                        acc += b_data[output_channel];
                        }
                        acc = MultiplyByQuantizedMultiplier(
                            acc, output_mult[output_channel],
                            output_shift[output_channel]);
                        acc += output_off;
                        acc = std::max(acc, output_activation_min);
                        acc = std::min(acc, output_activation_max);
                        o_data[(((batch * output_height + out_y) * output_width + out_x) * output_depth + output_channel)] = static_cast<int8_t>(acc);
                        //
                    }
                    }
                }
