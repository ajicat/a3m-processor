...
int main(){ //yes i know i should turn this into a function
    ...
    int layer_type = 1; //Pointwise=0, Depthwise=1
    int input_shape[] = {1, 48, 48, 8};
    int kernel_shape[] = {1, 3, 3, 8};
    int output_shape[] = {1, 48, 48, 8};
    ...
    int mem_w=w_tile_shape[1]*w_tile_shape[2]*tile_shape[3]*w_tile_shape[3];
    ...
    if (layer_type!=0){ //if depthwise
        while (tile_shape[3]>1&&(plm_w<mem_w||plm_out<mem_out)){
            ...
            mem_w=w_tile_shape[1]*w_tile_shape[2]*tile_shape[3]*w_tile_shape[3];
    ...
    std::cout<<output_shape[3]<<std::endl<<o_tile_shape[3]<<std::endl;
    ...
    int w_tile_size=w_tile_shape[0]*w_tile_shape[1]*w_tile_shape[2]*w_tile_shape[3];
    ...
    for(int batch=0; batch<kernel_shape[0]; batch++){
        ...
        for(int real_tile=0; real_tile<65; real_tile++){
            int ttile=real_tile+1;
            std::cout<<std::endl<<"Weight Tile "<<ttile<<":";
            if (layer_type!=0){
                for(int height=0; height<kernel_shape[1]; height++){
                    ...
                    for(int width=0; width<kernel_shape[2]; width++){
                        for(int channel=0; channel<w_tile_shape[3]; channel++){
                            ...
                            w_tile_data[((height)*kernel_shape[2]+width)*kernel_shape[3]+channel]=kernel_0[((batch*kernel_shape[1]+height)*kernel_shape[2]+width)*w_tile_shape[3]+channel+channel_off];
                            int pixel=w_tile_data[((height)*kernel_shape[2]+width)*kernel_shape[3]+channel];
                ...
                for(int height=0; height<kernel_shape[1]; height++){
                    ...
                    for(int width=0; width<kernel_shape[2]; width++){
                        for(int channel=0; channel<w_tile_shape[3]; channel++){
                            //w_tile_data[((batch*kernel_shape[1]+height)*kernel_shape[2]+width)*kernel_shape[3]+channel]=kernel_2[((batch*kernel_shape[1]+height)*kernel_shape[2]+width)*w_tile_shape[3]+channel];
                            int pixel=kernel_2[((batch*kernel_shape[1]+height)*kernel_shape[2]+width)*w_tile_shape[3]+channel];
                            std::cout<<std::uppercase<<std::hex<<pixel<<" "<<batch<<" ";
                        }
                    }
                }
            }
                ...
                for(int width=0; width<std::min(tile_shape[2], width_count+1); width++){
                    for(int channel=0; channel<std::min(tile_shape[3], channel_count+1); channel++){
                        ...
                        if((tile<height_number&&height-padding_up<0)||(tile%width_number==0&&width-padding_left<0)||(height+height_off+padding_down==padded_input_shape[1])||(width+width_off+padding_right==padded_input_shape[2])){
                            ...
                            pixel=image[((height+height_off-padding_up)*input_shape[2]*input_shape[3])+((width+width_off-padding_left)*input_shape[3])+channel+channel_off];
                        }
            ...
            for(int height=0; height<std::min(o_tile_shape[1], o_height_count); height++){
                ...
                for(int width=0; width<std::min(o_tile_shape[2], o_width_count); width++){
                    for(int channel=0; channel<std::min(o_tile_shape[3], o_channel_count); channel++){
                        ...
                        int o_channel_off;
                        if(layer_type!=0){
                            o_channel_off=real_tile/(tile_shape[1]*tile_shape[2]);
                        }
                        else{
                            o_channel_off=batch;
                        }
                        ...
                        pixel=image[((height+o_height_off)*output_shape[2]*output_shape[3])+((width+o_width_off)*output_shape[3])+channel+o_channel_off];
                        std::cout<<std::uppercase<<std::hex<<pixel<<" ";
                    }
                }
            }
