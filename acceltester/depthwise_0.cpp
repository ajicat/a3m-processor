...
int main(){
    ...
    int stride_h=2; //check
    int stride_w=2; //check
    int input_offset=1;
    int zero_point=-input_offset;
    int depth_multiplier=output_shape[3]/input_shape[3];
   ...
   //how many output channel iterations
    ...
    for(int batch=0; batch<kernel_shape[0]; batch++){
        ...
        for(int real_tile=0; real_tile<tile_number; real_tile++){
            //reset control
            uint8_t reset=0;
            dma_load_csr(&reset, CSR_BASE+0x30, 1);
            ...
            dma_load_csr(&input_offset, CSR_BASE+0x38, 4);
                ...
                for(int height=0; height<kernel_shape[1]; height++){
                    for(int width=0; width<kernel_shape[2]; width++){
                        for(int channel=0; channel<w_tile_shape[3]; channel++){
                            w_tile_data[((height)*kernel_shape[2]+width)*kernel_shape[3]+channel]=kernel_0[((batch*kernel_shape[1]+height)*kernel_shape[2]+width)*w_tile_shape[3]+channel];
                        }
                    }
                }
            }
            ...
            for(int channel=0; channel<output_shape[3]; channel++){
                ...
                bias_data[channel]=bias_0[channel+batch];
                qmult_data[channel]=mult_0[channel+batch];
                qshift_data[channel]=shift_0[channel+batch];
            }
            ...
            //register clear
            uint8_t clear=2;
            dma_load_csr(&clear, CSR_BASE+0x30, 1);
